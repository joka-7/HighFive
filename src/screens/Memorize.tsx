...
import { isReviewDay, WEEK_WORD_COUNT, weekLearningDays } from "../utils/cycle";
...
// Memorization (שינון) — the week's review, on Friday and Saturday. No new
// words: the 25 met on Sunday–Thursday come back, first as a self-test card
// list and then as a mixed recall quiz (English→Hebrew and Hebrew→English).
// Reachable earlier in the week too, over the words met so far.
...
export default function Memorize() {
  ...
  // Only days that have already happened — on a review day that is the full
  // week, earlier in the week it is what the learner has met so far.
  const days = useMemo(
    () => weekLearningDays().filter((d) => d <= today),
    [today],
  );
  ...
  if (!words) return <Spinner label="טוען את מילות השבוע..." />;

  if (words.length === 0) {
    return (
      <div className="card center">
        <h2>לא הצלחנו לטעון את מילות השבוע</h2>
  ...
  if (phase === "done") {
    return (
      <div className="celebrate">
        ...
        <p className="muted">
          {result.score}/{result.total} תשובות נכונות מתוך {words.length} מילות השבוע.
        </p>
        <button
    ...
    <div>
      ...
      >
        ...
        <p style={{ margin: "6px 0 0", opacity: 0.92 }}>
          {isReviewDay()
            ? `סוף שבוע — היום לא מוסיפים מילים חדשות, אלא מוודאים ש-${WEEK_WORD_COUNT} מילות השבוע נשארו.`
            : "אמצע השבוע — אפשר לשנן כבר עכשיו את המילים שנלמדו עד כה."}
        </p>
      </div>

      <div className="card">
        <div className="row-between">
          <span className="tag">{words.length} מילים מהשבוע</span>
